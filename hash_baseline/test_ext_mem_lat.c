    ...
    case DATAR:
        if (req->state == ALGO_REQ_NOT_FOUND) {
            ...
            if ((++finished_r_local) % 200000 == 0) {
                ...
                rd_start_ns = now;
                ftl_log("%lu th read request end. key: %s, iops: %lf, avg_lat: %ld, slow: %ld\n", finished_r_local, req->key.key, (double)200000 / elapsed, sum_rlat_ns_local / 200000, complete_r_slow);
